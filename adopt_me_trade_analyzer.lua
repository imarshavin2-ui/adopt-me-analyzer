...
for _, name in ipairs(OLD_GUI_NAMES) do
    local object = GuiParent:FindFirstChild(name)
...
BootGui.Name = "AM_ANALYZER_BOOT_V1162"
...
BootFrame.Size = UDim2.fromOffset(520, 80)
BootFrame.Position = UDim2.new(0.5, -260, 0, 90)
BootFrame.BackgroundColor3 = Color3.fromRGB(18, 21, 29)
...
BootText.Text = "ADOPT ME ANALYZER V11.6.2\nBOOT..."
...
local function setBoot(step, text, isError)
    ...
    BootText.TextColor3 =
        isError
        and Color3.fromRGB(255, 100, 110)
        or Color3.fromRGB(240, 243, 250)

    print("[AM V11.6.2]", step, text)
end

local function traceback(errorMessage)
    local result = tostring(errorMessage)
...
do
    local ok, err = xpcall(
        function()
            Fsys = require(RS:WaitForChild("Fsys"))

            assert(type(Fsys) == "table", "Fsys invalid")
            assert(type(Fsys.load) == "function", "Fsys.load missing")

            ClientData = Fsys.load("ClientData")
            ItemDB = Fsys.load("ItemDB")

            assert(type(ClientData) == "table", "ClientData invalid")
            assert(type(ItemDB) == "table", "ItemDB invalid")
        end,
        traceback
    )

    if not ok then
        setBoot("ERROR", err, true)
        return
    end
end

setBoot("2/8", "CLIENT DATA OK")
...
local function stroke(object, transparency)
    ...
    x.Color = Color3.fromRGB(58, 64, 78)
    x.Transparency = transparency or 0.35
    x.Thickness = 1
    x.Parent = object
    return x
end
...
local function label(
    ...
    x.TextXAlignment = alignment or Enum.TextXAlignment.Left
...
local function amRound(value, decimals)
    if type(value) ~= "number" then
        return nil
    end
...
local function getItemDB(item)
    ...
    local category = ItemDB[item.category]

    if type(category) ~= "table" then
        return nil
    end

    return category[item.kind]
end
...
local function getItemName(item)
    ...
    return tostring(item.kind or "Unknown Item")
end

local function getCategoryDisplay(item)
    local category = tostring(item.category or "unknown")

    return CATEGORY_DISPLAY[category] or category:upper()
end
...
local function getPetBaselessValue(entry, variant)
    if type(entry) ~= "table" then
        return nil, nil, "INVALID ENTRY"
    end
    ...
    if category == 13 then
        ...
        if not field then
            return nil, nil, "UNKNOWN VARIANT"
        end
...
local function getPetDemand(entry, variant)
    ...
    if variant:find("M", 1, true) then
        return entry.megaDemand
    end

    if variant:find("N", 1, true) then
        return entry.neonDemand
    end
...
local REQUEST = detectRequest()

print("[AMVGG] request =", type(REQUEST))
...
local function extractObject(body, startPosition)
    local depth = 0
...
local function parseBody(body)
    ...
    while cursor <= #body do
        ...
        local jsonText, ending =
            extractObject(
                body,
                position
            )

        if not jsonText or not ending then
            cursor = position + 5
            continue
        end
...
local function loadAMVGG()
    ...
    if
        categories.pets
        and next(categories.pets)
    then
        AMVGG.categories = categories
        AMVGG.counts = counts
        AMVGG.total = total

        AMVGG.ready = true
        AMVGG.version = AMVGG.version + 1
    ...
    AMVGG.ready = false
    AMVGG.error = "NO PET DATABASE"

    return false
end

--============================================================
-- ADOPT CATEGORY -> AMVGG
--============================================================
...
local function findAMVGG(item)
    ...
    if item.category == "pets" then
        ...
        -- Adopt Me eggs can live under pets
        if
            itemName:lower():find(
                "egg",
                1,
...
local function genericValue(entry)
    ...
    local value = num(entry.value)

    if value ~= nil then
        return value, "value"
    end

    value = num(entry.regularValue)

    if value ~= nil then
        return value, "regularValue"
    end

    value = num(entry.npRegularValue)

    if value ~= nil then
        return value, "npRegularValue"
    end
...
setBoot("4/8", "BUILDING GUI")

local Gui = Instance.new("ScreenGui")

Gui.Name = "AdoptMeTradeAnalyzerV1162"
...
local VersionLabel =
    label(
        Top,
        "V11.6.2",
        UDim2.fromOffset(72, 25),
        UDim2.new(0, 310, 0.5, -12),
        Enum.Font.GothamBold,
        10,
        C.ACCENT,
...
VersionLabel.BackgroundColor3 = Color3.fromRGB(35, 50, 80)

corner(VersionLabel, 6)
...
UIS.InputChanged:Connect(
    function(input)
        ...
        local x =
            dragOrigin.X.Scale * viewport.X
            + dragOrigin.X.Offset
            + delta.X

        local y =
            dragOrigin.Y.Scale * viewport.Y
            + dragOrigin.Y.Offset
            + delta.Y
...
local function setPage(name)
    ...
    for buttonName, navButton in pairs(Navigation) do
        if buttonName == name then
            navButton.BackgroundColor3 =
                Color3.fromRGB(48, 76, 130)

            navButton.TextColor3 = C.TEXT
        else
...
local AMVGGStatus =
    label(
        TradePage,
        "AMVGG: WAITING",
        UDim2.new(0, 700, 0, 20),
        UDim2.fromOffset(19, 54),
        Enum.Font.Code,
        9,
        C.YELLOW
    )
...
local StageLabel =
    label(
        TradePage,
        "NO ACTIVE TRADE",
        UDim2.fromOffset(165, 28),
        UDim2.new(1, -183, 0, 14),
        Enum.Font.GothamBold,
        10,
        C.MUTED,
        Enum.TextXAlignment.Center
...
TradeArea.Position = UDim2.fromOffset(16, 77)
TradeArea.Size = UDim2.new(1, -32, 1, -89)
TradeArea.BackgroundColor3 = C.PANEL
TradeArea.BorderSizePixel = 0
TradeArea.Parent = TradePage

corner(TradeArea, 10)
stroke(TradeArea, 0.35)
...
Center.Position = UDim2.new(0.435, 0, 0, 0)
Center.Size = UDim2.new(0.13, 0, 1, 0)
...
TradeDifference.TextWrapped = true
...
local function createOfferPanel(x)
    ...
    panel.Position = UDim2.new(x, 0, 0, 8)
    panel.Size = UDim2.new(0.425, 0, 1, -16)
...
local Yours = createOfferPanel(0.008)
local Theirs = createOfferPanel(0.567)
...
local function createSlot(side, index)
    ...
    local name =
        label(
            ...
            "EMPTY",
            UDim2.new(1, -10, 0, 34),
            UDim2.fromOffset(5, 17),
            Enum.Font.GothamBold,
            9,
            Color3.fromRGB(95, 101, 115),
            Enum.TextXAlignment.Center
        )
...
local function fillSlot(slot, item)
    ...
    local data = analyzeItem(item)

    slot.Analysis = data

    slot.Name.Text = data.name
    slot.Name.TextColor3 = C.TEXT

    slot.Variant.Text = data.variant
    slot.Variant.TextColor3 =
        variantColor(data.variant)

    slot.Category.Text = data.category
    ...
    if data.value ~= nil then
        ...
        slot.Value.TextColor3 = C.ACCENT
...
local function updateSide(side, offer)
    ...
    for i = 1, 18 do
        ...
        if item then
            ...
            clearSlot(side.Slots[i])
        end
    end

    if count == 0 then
        side.Total.Text = "TOTAL: 0"
        side.Total.TextColor3 = C.MUTED
    ...
    elseif missing == 0 then
        ...
        side.Total.TextColor3 = C.ACCENT
        ...
        side.Total.TextColor3 = C.YELLOW
    end
    ...
    then
        side.Ready.Text = "CONFIRMED"
        side.Ready.TextColor3 = C.GREEN

    elseif
        type(offer) == "table"
        and offer.negotiated == true
    then
        side.Ready.Text = "ACCEPTED"
        side.Ready.TextColor3 = C.GREEN

    else
        side.Ready.Text = "WAITING"
        side.Ready.TextColor3 = C.MUTED
    end
...
local function offerSignature(offer)
    ...
    result[#result + 1] =
        tostring(offer.negotiated)

    result[#result + 1] =
        tostring(offer.confirmed)
...
local function calculateResult(mine, theirs)
    if
        mine.missing > 0
        or theirs.missing > 0
    then
        TradeResult.Text = "INCOMPLETE"
        TradeResult.TextColor3 = C.YELLOW
        TradeDifference.Text = "missing values"
        return
    end
    ...
    if
        ...
        and theirs.count == 0
    then
        TradeResult.Text = "WAIT"
        TradeResult.TextColor3 = C.MUTED
    ...
    if percent <= FAIR_PERCENT then
        TradeResult.Text = "FAIR"
        TradeResult.TextColor3 = C.YELLOW

    elseif difference > 0 then
        TradeResult.Text = "WIN"
        TradeResult.TextColor3 = C.GREEN

    else
        TradeResult.Text = "LOSE"
        TradeResult.TextColor3 = C.RED
    end
end
...
local function noTrade()
    Yours.Name.Text = LocalPlayer.Name
    Theirs.Name.Text = "NO PARTNER"

    StageLabel.Text = "NO ACTIVE TRADE"
...
local function updateTrade()
    ...
    if isMe(trade.sender) then
        myOffer = trade.sender_offer
        theirOffer = trade.recipient_offer

        me = trade.sender
        partner = trade.recipient
    else
        myOffer = trade.recipient_offer
        theirOffer = trade.sender_offer

        me = trade.recipient
        partner = trade.sender
    end

    local signature =
        tostring(trade.trade_id)
        .. ":"
        .. tostring(trade.current_stage)
        .. ":"
        .. offerSignature(myOffer)
        .. ":"
        .. offerSignature(theirOffer)
        .. ":DB:"
        .. tostring(AMVGG.version)
    ...
    Yours.Name.Text = playerName(me)
    Theirs.Name.Text = playerName(partner)
...
SearchBox.PlaceholderText = "Pilot Gull"
SearchBox.PlaceholderColor3 = C.MUTED
SearchBox.TextColor3 = C.TEXT
SearchBox.Font = Enum.Font.Code
SearchBox.TextSize = 11
SearchBox.ClearTextOnFocus = false
SearchBox.Parent = ValuesPage
...
ResultBox.BackgroundColor3 = C.PANEL
ResultBox.BorderSizePixel = 0
ResultBox.TextColor3 = C.TEXT
...
ResultBox.Text = "Waiting for AMVGG..."
ResultBox.Parent = ValuesPage
...
local function searchValues()
    if not AMVGG.ready then
        ResultBox.Text = "AMVGG NOT READY"
        return
    end
    ...
    if query == "" then
        ResultBox.Text = "Enter item name"
        return
    end
    ...
    local lines = {
        ...
        "REGULAR BASE = "
        .. tostring(found.regularValue),

        "NEON BASE = "
        .. tostring(found.neonValue),

        "MEGA BASE = "
        .. tostring(found.megaValue),
...
SearchButton.Activated:Connect(searchValues)
...
local UpdateText =
    label(
        ...
        "V11.6.2\n\n"
        .. "• REAL AMVGG category multipliers\n"
        .. "• Uses AMVGG category\n"
        .. "• NP/R/F calculated from Regular\n"
        .. "• N/NR/NF calculated from Neon\n"
        .. "• M/MR/MF calculated from Mega\n"
        .. "• FR/NFR/MFR raw base values\n"
        .. "• Category 11 special rules\n"
        .. "• Category 13 exact fields\n"
        .. "• AMVGG rounding\n"
        .. "• 18 trade slots\n"
        .. "• Dark Chocobunny alias\n\n"
...
OpenButton.InputBegan:Connect(
    function(input)
        ...
        then
            ...
            floatOrigin = OpenButton.Position
        end
    end
)
...
UIS.InputChanged:Connect(
    function(input)
        ...
        local x =
            floatOrigin.X.Scale * viewport.X
            + floatOrigin.X.Offset
            + delta.X

        local y =
            floatOrigin.Y.Scale * viewport.Y
